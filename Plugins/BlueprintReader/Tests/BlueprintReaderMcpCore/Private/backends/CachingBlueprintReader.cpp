...
nlohmann::json CachingBlueprintReader::DiffAsset(
	std::string_view a, std::string_view b, std::string_view depth) {
	return inner_->DiffAsset(a, b, depth);
}

nlohmann::json CachingBlueprintReader::PrepareMerge(
...
nlohmann::json CachingBlueprintReader::EndBatch(bool skipCompile, bool rollback,
												bool saveOnError) {
	nlohmann::json flushAck = inner_->EndBatch(skipCompile, rollback, saveOnError);
...
nlohmann::json CachingBlueprintReader::UiFocusTab(std::string_view tabLabel) {
