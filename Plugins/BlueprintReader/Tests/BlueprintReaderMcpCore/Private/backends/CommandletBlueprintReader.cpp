...
nlohmann::json CommandletBlueprintReader::UiType(std::string_view widgetPath,
		std::string_view text, std::string_view expectType) {
	std::vector<std::wstring> args;
	args.push_back(L"-Op=UiType");
	args.push_back(L"-WidgetPath=" + Widen(widgetPath));
	args.push_back(L"-Text=" + Widen(text));
	if (!expectType.empty()) { args.push_back(L"-ExpectType=" + Widen(expectType)); }
	return RunOp(args);
}
...
CommandletBlueprintReader::ListAutomationTests() {
	auto j = RunOp({L"-Op=ListAutomationTests"});
...
CommandletBlueprintReader::GetProjectSettingValues(std::string_view classPath) {
	...
	ProjectSettingValuesResult out;
	if (j.is_object()) {
		out.valid     = j.value("valid", false);
		out.classPath = j.value("class_path", std::string{});
		if (auto it = j.find("values"); it != j.end() && it->is_array()) {
			for (const auto& v : *it) {
				if (!v.is_object()) continue;
				SettingValueInfo info;
				info.name  = v.value("name",  std::string{});
				info.value = v.value("value", std::string{});
				info.type  = v.value("type",  std::string{});
				out.values.push_back(std::move(info));
			}
		}
	}
	return out;
...
CommandletBlueprintReader::ListProjectSettings() {
	auto j = RunOp({L"-Op=ListProjectSettings"});
...
CommandletBlueprintReader::GetRecentlySavedPackages() {
	auto j = RunOp({L"-Op=GetRecentlySavedPackages"});
...
CommandletBlueprintReader::GetStreamingSources() {
	auto j = RunOp({L"-Op=GetStreamingSources"});
...
CommandletBlueprintReader::GetWatchedPins(std::string_view assetPath) {
	...
	WatchedPinsResult out;
	if (j.is_object()) {
		out.valid = j.value("valid", false);
		if (auto it = j.find("pins"); it != j.end() && it->is_array()) {
			for (const auto& p : *it) {
				if (!p.is_object()) continue;
				WatchedPinInfo info;
				info.pinName   = p.value("pin_name",  std::string{});
				info.nodeGuid  = p.value("node_guid", std::string{});
				info.nodeName  = p.value("node_name", std::string{});
				info.direction = p.value("direction", std::string{});
				out.pins.push_back(std::move(info));
			}
		}
	}
	return out;
...
CommandletBlueprintReader::GetEditorEvents() {
	auto j = RunOp({L"-Op=GetEditorEvents"});
