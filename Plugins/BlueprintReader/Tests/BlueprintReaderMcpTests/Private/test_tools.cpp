...
// TEST-2 P1b: editor-only UI click — the mock throws a clear not-supported error.
TEST_CASE("ui_click: mock backend throws not-supported") {
	Fixture f;
	CHECK_THROWS_WITH_AS(
...
// TEST-2 P1b: editor-only dock-tab focus — the mock throws a clear not-supported error.
TEST_CASE("ui_focus_tab: mock backend throws not-supported") {
	Fixture f;
	CHECK_THROWS_WITH_AS(
...
// TEST-2 P1b: editor-only menu invocation — the mock throws a clear not-supported error.
TEST_CASE("ui_invoke_menu: mock backend throws not-supported") {
	Fixture f;
	CHECK_THROWS_WITH_AS(
		f.Call("ui_invoke_menu", json{{"menu", "LevelEditor.MainMenu.Window"}, {"entry", "OutputLog"}}),
		doctest::Contains("requires the live backend"),
		bpr::backends::BlueprintReaderError);
}
