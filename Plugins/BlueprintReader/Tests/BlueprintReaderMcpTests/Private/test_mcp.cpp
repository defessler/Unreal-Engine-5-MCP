...
TEST_CASE("MCP handshake + tools/list + tools/call list_blueprints") {
	...
	CHECK(list.size() == 258);  // +25 P8 + 12 P11 + 11 P12 Wave 2 + 18 P13 Wave 3 + 24 P14 + 20 P17 + 5 P16 + 1 P10 + 2 clone_graph/implement_interface + 1 read_actor_instance + 2 create_material/create_material_instance
...
TEST_CASE("Lazy discovery: tool search mode advertises just 4 tools but call_tool reaches all") {
	...
	tools::ToolRegistry registry;
	tools::RegisterBlueprintTools(registry, reader);
	REQUIRE(registry.TotalRegistered() == 258);

	tools::RegisterToolsetMetaTools(registry);
	REQUIRE(registry.TotalRegistered() == 261);  // 258 + 3 meta-tools
