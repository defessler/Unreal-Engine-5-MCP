...
TEST_CASE("MCP handshake + tools/list + tools/call list_blueprints") {
	...
	CHECK(list.size() == 264);  // +25 P8 + 12 P11 + 11 P12 Wave 2 + 18 P13 Wave 3 + 24 P14 + 20 P17 + 5 P16 + 1 P10 + 2 clone_graph/implement_interface + 1 read_actor_instance + 2 create_material/create_material_instance + 2 diff_asset/prepare_merge + 1 health_check (UX-P4a) + 2 describe_k2node/generate_k2node_skeleton (EDIT-5) + 1 ui_list_widgets (TEST-2 P0)
...
TEST_CASE("Lazy discovery: tool search mode advertises just 4 tools but call_tool reaches all") {
	...
	tools::ToolRegistry registry;
	tools::RegisterBlueprintTools(registry, reader);
	REQUIRE(registry.TotalRegistered() == 264);

	tools::RegisterToolsetMetaTools(registry);
	REQUIRE(registry.TotalRegistered() == 267);  // 264 + 3 meta-tools
	...
	// describe_toolset(name="cpp") — should list the 8 cpp tools
	// (7 + generate_k2node_skeleton from EDIT-5)
	json req2 = {
