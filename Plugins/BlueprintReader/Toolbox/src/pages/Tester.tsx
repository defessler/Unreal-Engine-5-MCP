...
import {
  resolveTemplate, coerceArgs, isValidJsonOfType, validateArgs,
...
// TBX-P7: the catalog is generated by Dump-Tools.ps1 and force-cast above. If its
// shape drifts the Tester would break silently — fail loudly in the console
// instead (cheap runtime guard, no schema dependency).
if (!Array.isArray(tools) || (tools.length > 0 && (!tools[0]?.name || !tools[0]?.input_schema))) {
  console.error('[Tester] tools.json drift: unexpected catalog shape — re-run Dump-Tools.ps1. The tool list may be broken.');
}

// TBX-P3: persist the call history across sessions (cap 50; the prior in-memory
// list was lost on navigation/restart).
const HISTORY_KEY = 'bpr-tester-history';
const HISTORY_CAP = 50;
function loadHistoryStore(): HistoryEntry[] {
  try {
...
export default function Tester() {
  ...
  const [history, setHistory] = useState<HistoryEntry[]>(loadHistoryStore);
  ...
  // TBX-P3: persist call history across sessions.
  useEffect(() => {
    try { localStorage.setItem(HISTORY_KEY, JSON.stringify(history)); } catch { /* quota/disabled */ }
  }, [history]);
  ...
  async function sendCall() {
    ...
    try {
      ...
      setResult(r);
      setHistory(prev => [{ tool: selectedTool.name, args, result: r, elapsed, ts: Date.now() }, ...prev.slice(0, HISTORY_CAP - 1)]);
    } catch (e) {
  ...
  // TBX-P3: download the full persisted history as JSON.
  const exportHistory = useCallback(() => {
    const blob = new Blob([JSON.stringify(history, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = 'bpr-tester-history.json';
    a.click();
        ...
        {mode === 'single' && history.length > 0 && (
          <div className="border-t border-ue-border">
            <div className="px-3 py-1.5 flex items-center justify-between">
              <span className="text-xs text-gray-500 uppercase tracking-wider">Recent ({history.length})</span>
              <div className="flex gap-2">
                {/* TBX-P3: export the full persisted history; clear it. */}
                <button onClick={exportHistory} title="Export history as JSON"
                  className="text-xs text-gray-500 hover:text-gray-300">Export</button>
