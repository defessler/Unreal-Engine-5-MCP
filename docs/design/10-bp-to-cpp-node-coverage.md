...
| Async tasks (UBlueprintAsyncActionBase: `K2Node_BaseAsyncTask`, `K2Node_AsyncAction`, `K2Node_LatentAbilityCall`, `K2Node_LatentGameplayTaskCall`) | Same continuation-passing shape as Delay, but with named multicast-delegate output execs (`OnSuccess`/`OnFailed`/etc.) and typed payload pins per delegate. | Now emits a **rich async-task sidecar** with: `factory_function` + `factory_class` (the `<TaskClass>::Create(...)` call), `factory_args` (each input pin's expression), `wired_callbacks` (per wired output exec: pin name + suggested `On<NodeTitle><PinName>` UFUNCTION name), `output_data_pins` (every output data pin's name + type, to wire into matching callback signatures), and `cpp_recipe` (the canonical 4-line C++ pattern). Full auto-lowering blocked on walker entry-pin tracking + per-delegate-signature param introspection. |
