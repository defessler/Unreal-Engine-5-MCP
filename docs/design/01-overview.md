...
bp-reader is a Blueprint introspection and mutation surface for UE 5.x
(built against pre-5.8 and UE 5.8; 5.8-only engine APIs must be guarded
with `UE_VERSION_OLDER_THAN`),
...
├── Source/
│   ├── BlueprintReaderEditor/           UE editor plugin (UnrealEd; multi-engine: pre-5.8 + 5.8)
...
## 268 tools across 23 categories (authoritative count: generated docs/TOOLS.md)

The MCP server registers 268 tools at startup. The tool count and
per-category breakdown are maintained in the generated catalog
`docs/TOOLS.md` (regenerated by `Scripts/Dump-Tools.ps1`; CI's
`Dump-Tools.ps1 -Check` drift-gates it). The test that pins the count
is `Plugins/BlueprintReader/Tests/BlueprintReaderMcpTests/Private/test_tools.cpp`
(~line 60: `spec.size() == 268`); it is also pinned in
...
For the current per-category breakdown, see `docs/TOOLS.md` — the
generated catalog is the single source of truth and never drifts.
...
with the rest in `ApplyOps.cpp` (`apply_ops`, `preview_ops`) and
`CompileFunction.cpp` (`compile_function`).

Don't enumerate them here — call `tools/list` against the running
server, or read `docs/TOOLS.md` / `wiki/Tool-Reference.md`. Adding a
new tool means plumbing through both halves; the checklist is in
