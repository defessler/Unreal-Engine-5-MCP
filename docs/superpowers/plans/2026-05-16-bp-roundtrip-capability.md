...
### Task 21: BPIR roundtrip — `BP_Enemy` smoke (stages 1-3 only)

**Files:**
- Create: `Plugins/BlueprintReader/Tests/BlueprintReaderMcpTests/Private/test_roundtrip_bpir.cpp`
- ~~Create: `Plugins/BlueprintReader/Tests/BlueprintReaderMcpTests/fixtures/BP_TestEnemy_bpir.json` (generated)~~ — **SKIPPED**: BPIRRoundtrip stages 4-5 (parse/transpile) are stubbed, so no `bpir_after` exists to golden-compare. Add when whole-class parse lands.

> **Scope note (May 2026):** the BPIRRoundtrip library implements stages 1-3 only (decompile -> emit C++ -> write to disk). Stages 4-5 record `failing_stage="parse"` with a deferred-implementation message — see `BPIRRoundtrip.cpp` lines ~195-214. The test below validates stages 1-3 succeed and asserts the documented stub-failure shape; tighten when parse/transpile land. Fixture name corrected: the seed produces `BP_Enemy` (at `/Game/AI/BP_Enemy`), not `BP_TestEnemy`.

- [x] **Step 1: Write the smoke test (stages 1-3, deferred-stub assertion for 4-5)**
...
```bash
Binaries/Win64/BlueprintReaderMcpTests.exe -tc="*BP_Enemy*"
```

Expected: pass on stages 1-3 (decompile + emit + write). Stages 4-5 are stubbed; assertions verify the `failing_stage == "parse"` shape.
...
git commit -m "test(roundtrip): BPIR smoke — BP_Enemy stages 1-3"
```
