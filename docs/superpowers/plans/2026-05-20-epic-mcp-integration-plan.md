...
| 11 | **H Tier 1** PluginToolset + GameFeatures + GAS **(11 of 12 — reads COMPLETE + GFP activate_game_feature/deactivate_game_feature SHIPPED; only PluginToolset .uplugin descriptor writes pending)** | 10–12 | 68d | Med (High for Lyra users) | Lyra value-add |
...
| 14 | **EA-pull Wave 4** World + SCC + system state **(11 of ~25 SHIPPED — +batch 3 SCC detail: get_source_control_status/get_file_lock_status)** | 6–8 | 91d | Low (Med for Lyra: partition+data layers) | Partition, data layers, SCC, outliner, recent activity |
...
| 17 | **EA-pull Wave 5** Advanced + niche **(4 of ~20 SHIPPED — +get_monitor_info/get_live_coding_state)** | 5–7 | 113d | Very Low — demand-driven | BP debug, takes, render queue, modeling tools, console history |
