...
| 9 | **C3–C5** HTTP + Sessions + SSE **(C3+C5 SHIPPED + live-validated — HttpServerMain thread-per-connection socket loop, BP_READER_HTTP_PORT opt-in. POST round-trips JSON-RPC (200), Origin guard (403); GET /mcp = chunked text/event-stream SSE holding the socket open, draining Server::TakePendingNotifications into FormatSseFrame chunks + keep-alives. Only C4 Mcp-Session-Id remains)** | 13–15 | 46d | Very High — gates Push events entirely | Transport for push events |
