...
| 9 | **C3–C5** HTTP + Sessions + SSE **(✅ FUNCTIONALLY COMPLETE + live-validated — HttpServerMain thread-per-connection socket loop, BP_READER_HTTP_PORT opt-in. C3 POST JSON-RPC (200) + Origin guard (403); C4 Mcp-Session-Id assigned on initialize (128-bit, lenient validation); C5 GET /mcp chunked text/event-stream draining Server::TakePendingNotifications → FormatSseFrame + keep-alives. cpp-httplib never needed. Refinements: strict session validation, multi-client SSE fan-out)** | 13–15 | 46d | Very High — gates Push events entirely | Transport for push events |
| 10 | **EA-push Wave 1** Core event notifications **(push pipeline END-TO-END + validated — editor/subscribe + experimental.editor cap; UE delegates (selection/asset-open/PIE) → editor ring buffer → SSE stream auto-push (drains reader->GetEditorEvents under the server mutex, queues subscribed notifications/editor/* → emits as SSE frames). SSE survives the poll with mock (live-smoke). 16 events wired — 13 Tier-A (selection, asset_opened/removed/renamed, pie_started/stopped/paused/resumed, package_saved, map_opened, actor_added/deleted, blueprint_compiled) + 1 Tier-B debounced (actor_property_changed ~2Hz) + 2 Tier-C debounced (camera_moved 4Hz, dirty_packages_changed 1Hz). Per-session SSE fan-out DONE (unit-tested — each client gets its own copy, no theft). Remaining: niche Lyra GFP events (need a UObject IGameFeatureStateChangeObserver) + partition-cell events (live-editor-only to exercise))** | 8–10 | 56d | Very High — biggest agentic differentiator vs Epic | Tier-A SSE events on user actions |
...
| 12 | **EA-pull Wave 2** Per-asset-editor selection **(SURFACE COMPLETE — all 11 listed tools shipped: 8 functional + 3 documented v1 stubs (anim/niagara/curve_editor), same standard as Phase 14/17 stubs. The 3 stubs' real selection data is unreachable even from the in-editor live backend without an engine-build change: `IAssetEditorInstance*` can't reach the concrete toolkit (sibling cross-cast needs `dynamic_cast` — engine is no-RTTI — and the toolkit types are module-private with no exported header). Documented as an engine-RTTI follow-up.)** | 6–8 | 76d | Med — drills into specific workflows | BP/Material/Sequencer/Anim/UMG selection state |
...
| 16 | **H Tier 2** ConfigSettings + Automation **(4 of ~8 SHIPPED — ConfigSettings get/set core + list_automation_tests; pairs with existing run_automation_tests)** | 6–8 | 106d | Low (Med for Lyra) | More Lyra value-add |
