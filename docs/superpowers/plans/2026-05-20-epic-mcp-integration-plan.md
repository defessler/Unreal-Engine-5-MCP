...
| 10 | **EA-push Wave 1** Core event notifications **(push pipeline END-TO-END + validated — editor/subscribe + experimental.editor cap; UE delegates (selection/asset-open/PIE) → editor ring buffer → SSE stream auto-push (drains reader->GetEditorEvents under the server mutex, queues subscribed notifications/editor/* → emits as SSE frames). SSE survives the poll with mock (live-smoke). 12 Tier-A events wired (selection, asset_opened, pie_started/stopped/paused/resumed, package_saved, map_opened, asset_removed, asset_renamed, actor_added, actor_deleted). Remaining: per-session SSE fan-out + Tier-B/C debounced events (event emission only exercisable with a live editor))** | 8–10 | 56d | Very High — biggest agentic differentiator vs Epic | Tier-A SSE events on user actions |
