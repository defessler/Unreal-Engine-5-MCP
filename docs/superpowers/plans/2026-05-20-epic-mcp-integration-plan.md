...
| 10 | **EA-push Wave 1** Core event notifications **(push pipeline END-TO-END + validated — editor/subscribe + experimental.editor cap; UE delegates (selection/asset-open/PIE) → editor ring buffer → SSE stream auto-push (drains reader->GetEditorEvents under the server mutex, queues subscribed notifications/editor/* → emits as SSE frames). SSE survives the poll with mock (live-smoke). 7 Tier-A events wired (selection, asset_opened, pie_started/stopped/paused/resumed, package_saved). Remaining: ~4 more event types (asset_closed/level_changed/blueprint_compiled/asset_added — mechanical), per-session fan-out)** | 8–10 | 56d | Very High — biggest agentic differentiator vs Epic | Tier-A SSE events on user actions |
