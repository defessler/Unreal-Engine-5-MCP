...
| 10 | **EA-push Wave 1** Core event notifications **(subscription model SHIPPED + live-validated — editor/subscribe + editor/unsubscribe + experimental.editor capability, EditorSubscriptions registry, BP_READER_PUSH_EVENTS opt-in. Validated over HTTP: initialize advertises the cap, subscribe→id, unsubscribe→ok. Tier-A event SOURCES — editor UE-delegate → QueueNotification over the live channel — pending)** | 8–10 | 56d | Very High — biggest agentic differentiator vs Epic | Tier-A SSE events on user actions |
