...
| 9 | **C3–C5** HTTP + Sessions + SSE **(C3 SHIPPED — HttpServerMain localhost socket loop, BP_READER_HTTP_PORT opt-in; POST round-trips JSON-RPC live (200), Origin guard enforced (403), GET 405; message layer + dispatch already done in Phase A. C4 Mcp-Session-Id + C5 SSE-GET streaming pending)** | 13–15 | 46d | Very High — gates Push events entirely | Transport for push events |
