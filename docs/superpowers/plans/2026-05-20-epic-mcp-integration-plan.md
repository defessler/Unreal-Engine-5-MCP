...
| 10 | **EA-push Wave 1** Core event notifications **(push pipeline END-TO-END + validated — editor/subscribe + experimental.editor cap; UE delegates (selection/asset-open/PIE) → editor ring buffer → SSE stream auto-push (drains reader->GetEditorEvents under the server mutex, queues subscribed notifications/editor/* → emits as SSE frames). SSE survives the poll with mock (live-smoke). Remaining: +8 more Tier-A event types (mechanical delegate adds), per-session fan-out)** | 8–10 | 56d | Very High — biggest agentic differentiator vs Epic | Tier-A SSE events on user actions |
