...
| 10 | **EA-push Wave 1** Core event notifications **(subscription model + event-source capture SHIPPED — editor/subscribe + editor/unsubscribe + experimental.editor cap; get_editor_events drains a delegate-fed buffer (selection/asset-open/PIE start-stop) captured editor-side via lazy-subscribed UE delegates. Remaining: SSE auto-push (drain → QueueNotification) so events flow without polling)** | 8–10 | 56d | Very High — biggest agentic differentiator vs Epic | Tier-A SSE events on user actions |
