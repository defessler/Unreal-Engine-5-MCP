...
| 10 | **EA-push Wave 1** Core event notifications **(push pipeline END-TO-END + validated — editor/subscribe + experimental.editor cap; UE delegates (selection/asset-open/PIE) → editor ring buffer → SSE stream auto-push (drains reader->GetEditorEvents under the server mutex, queues subscribed notifications/editor/* → emits as SSE frames). SSE survives the poll with mock (live-smoke). 13 Tier-A events + 1 Tier-B debounced (actor_property_changed, ~2/sec gate) wired: selection, asset_opened/removed/renamed, pie_started/stopped/paused/resumed, package_saved, map_opened, actor_added/deleted, blueprint_compiled. Per-session SSE fan-out DONE (unit-tested — each client gets its own copy, no theft). Remaining: more Tier-C debounced + Lyra GFP/partition events (live-editor-only to exercise))** | 8–10 | 56d | Very High — biggest agentic differentiator vs Epic | Tier-A SSE events on user actions |
