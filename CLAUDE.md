...
| `BP_READER_BACKUP`           | `1` (on)                           | REL-4: before the FIRST save of each asset per session, copy the on-disk `.uasset` to `Saved/BPReaderBackups/<Package>-<UTC>.uasset` (newest 5 kept per asset). Restore = copy the backup over the asset file while no editor holds it. Set `0` to disable. |
...
7. **Tool count assertion** — bump the single `bpr::test::kExpectedToolCount`
   constant in `test_helpers.h`; every count check across the suite
   (`test_tools`, `test_mcp`, `test_phase_d`, `test_progressive_default`,
   `test_protocol_compat`, `test_tool_smoke_live`) references it, so it's one
   edit, not seven.
