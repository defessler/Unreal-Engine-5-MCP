...
Edit `~/.claude/mcp.json` (or per-project `.mcp.json`):
...
Same shape, in `%APPDATA%\Claude\claude_desktop_config.json` under the
`mcpServers` key. Restart Claude Desktop after editing.
